           ...
           <div className="flex flex-wrap gap-2 mt-1">
             {isFullyClosed ? (
               ...
               <>
                 ...
                 ) : womenAvailable > 0 && womenAvailable <= 3 ? (
                   ...
                   if (isEmpty) {
                     return (
                       <div key={i} className="flex items-center gap-2">
                         ...
                         <span className={`font-medium tracking-tight ${isMenClosed ? 'text-gray-400' : (menAvailable > 0 && menAvailable <= 3) ? 'text-blue-500 font-bold' : 'text-blue-500'}`}>
                ...
                {cohort.womenList.map((item: string, i: number) => {
                   ...
                   if (isEmpty) {
                     return (
                        <div key={i} className="flex items-center gap-2">
                          <span className="w-5 text-center font-bold text-gray-300">{num}</span>
                          <span className={`font-medium tracking-tight ${isWomenClosed ? 'text-gray-400' : (womenAvailable > 0 && womenAvailable <= 3) ? 'text-pink-500 font-bold' : 'text-pink-500'}`}>
