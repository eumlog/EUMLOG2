        ...
        <>
            <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled || mobileMenuOpen ? 'bg-white/95 backdrop-blur-md border-b border-gray-100 text-black shadow-sm' : 'text-white'}`}>
                <div className="max-w-[1000px] w-[92%] mx-auto py-5 md:py-6 flex justify-between items-center">
                    ...
                    <div className="hidden md:flex items-center gap-5">
                        ...
                        <a href="https://naver.me/G4GlQVbi" target="_blank" rel="noopener noreferrer" className={`px-5 py-2 rounded-full text-[12px] font-black transition-all duration-300 border ${isScrolled ? 'bg-eum-dark text-white border-eum-dark hover:bg-white hover:text-eum-dark' : 'bg-white/10 text-[#D4D4C8] border-[#D4D4C8]/30 hover:bg-[#D4D4C8] hover:text-eum-dark'}`}>
                            신청하기
                        </a>
                    </div>
            ...
            <div className="mobile-menu fixed inset-0 bg-white z-40 transform translate-x-full opacity-0 flex flex-col justify-center items-center gap-8 text-lg font-bold text-eum-dark lg:hidden px-8 text-center">
                ...
                <div className="flex flex-col gap-4 mt-8 items-center w-full">
                    ...
                    <a href="https://naver.me/G4GlQVbi" target="_blank" rel="noopener noreferrer" onClick={() => setMobileMenuOpen(false)} className="w-full max-w-xs py-4 bg-eum-dark text-white rounded-full text-sm font-black shadow-xl">
                        매칭 신청하기
                    </a>
                </div>
            </div>
